...
import type { AppType } from '../../../src/http/app.ts'
...
export async function fetchEvents(
  ...
  try {
    ...
    if (!response.ok) throw new ApiError(response.status, t('loadFailed', {
...
export async function fetchMe(signal: AbortSignal): Promise<MeDto> {
  try {
    const response = await client.api.me.$get(undefined, { init: { signal } })
    if (!response.ok) throw new ApiError(response.status, t('loadFailed', {
      status: response.status,
    }))
    return await response.json()
...
export async function logout(): Promise<void> {
  try {
    ...
    if (!response.ok) throw new ApiError(response.status, t('signOutFailed'))
  } catch (error) {
    if (error instanceof ApiError) throw error
