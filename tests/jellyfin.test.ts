...
Deno.test('Jellyfin login identifies as calthing and validates each session', async () => {
  ...
  assertEquals(await client.authenticate('alice', 'wrong', 'browser-id'), null)
  const session = await client.authenticate('alice', 'right', 'browser-id')
  ...
  assertEquals(await client.user('jf-token'), { id: 'user-1', name: 'alice', avatarUrl })
  assertEquals(requests[5]?.url.pathname, '/base/Users/Me')
})
